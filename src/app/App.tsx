      ...
      <div className="px-2 lg:px-3 pb-4 border-t border-white/[0.05] pt-3 space-y-2">
        ...
        <button
          ...
          className={cn(
            ...
            active === "account"
              ? "bg-[#2A74C4]/15 text-[#7BB8F0] border-l-2 border-[#2A74C4] pl-[4.5px] lg:pl-[6.5px]"
              : "text-white/45 hover:text-white/75 hover:bg-white/[0.04]"
          )}
        >
...
const OB_SLIDES = [
  ...
  {
    title: "Consult it with Personal AI Agent",
    sub: "Find the best solutions and insight about your ideas based on data!",
  },
  {
...
function OnboardingView({ onFinish }: { onFinish: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-[#F4F6F8] flex items-center justify-center p-0 sm:p-6 font-['Plus_Jakarta_Sans',sans-serif]">
      {/* Desktop simulated phone wrapper / Responsive container */}
      <div className="w-full max-w-[420px] h-[100dvh] sm:h-[840px] sm:rounded-[40px] sm:shadow-[0_32px_64px_rgba(15,15,15,0.08)] sm:border sm:border-gray-100/60 bg-[#F7F8FA] flex flex-col justify-between overflow-hidden relative">
        
        {/* Top Header Row */}
        <div className="flex justify-end px-6 pt-4 sm:pt-6 pb-1 sm:pb-2 z-10 shrink-0">
          <button 
            onClick={onFinish} 
            className="text-[14px] font-semibold text-[#B1B5C4] hover:text-[#141718] active:scale-95 transition-all cursor-pointer font-['Poppins',sans-serif]"
          >
            Skip
          </button>
        </div>

        {/* Center Card Presentation Area */}
        <div className="flex-1 flex flex-col justify-center px-6 py-1 xs:py-2 overflow-hidden">
          {/* Main Visual Frame with responsive dynamic scaling height and fixed aspect ratio */}
          <div className="h-[36vh] xs:h-[38vh] sm:h-[420px] max-h-[420px] min-h-[160px] aspect-[3.2/4] mx-auto rounded-[33.5px] shadow-[0_24px_50px_-10px_rgba(15,15,15,0.12)] bg-white relative overflow-hidden transition-all duration-300 hover:scale-[1.01] shrink-0">
            <BusinessmanIllustration slide={slide} />
          </div>

          {/* Slide Indicators / Pagination Dots */}
          <div className="flex justify-center items-center gap-1.5 mt-4 sm:mt-8 mb-3 sm:mb-6 shrink-0">
            {OB_SLIDES.map((_, i) => (
              <button
                key={i}
                onClick={() => setSlide(i)}
                className={cn(
                  ...
                  i === slide
                    ? "w-2.5 h-2.5 bg-[#141718] ring-4 ring-[#141718]/15"
                    : "w-2 h-2 bg-[#B1B5C4]/60 hover:bg-[#141718]/40"
                )}
              />
            ))}
          </div>

          {/* Onboarding Titles & Subtitle Texts */}
          <div className="text-center px-4 max-w-[340px] mx-auto select-none shrink-0">
            <h2 className="text-xl xs:text-2xl font-bold text-[#23262F] tracking-tight leading-[1.25] font-['Plus_Jakarta_Sans'] mb-1.5 sm:mb-2.5">
              {s.title}
            </h2>
            <p className="text-[#8E9295] text-[12px] xs:text-[13.5px] leading-relaxed font-normal font-['Inter'] px-2">
              {s.sub}
            </p>
          </div>
        </div>

        {/* Bottom Navigator Pill Component */}
        <div className="pb-4 xs:pb-8 pt-1 sm:pt-2 px-6 flex justify-center shrink-0">
          <div className="bg-[#FCFCFD] border border-[#E6E8EC]/80 shadow-[0_16px_36px_rgba(15,15,15,0.06)] rounded-[18px] p-3.5 sm:p-4 flex items-center gap-7 transition-all">
            {/* Back Button */}
            <button
              onClick={() => slide > 0 && setSlide(slide - 1)}
              disabled={slide === 0}
              className={cn(
                "p-1 hover:scale-105 active:scale-95 transition-all cursor-pointer flex items-center justify-center",
                slide === 0 ? "text-[#E6E8EC] cursor-not-allowed" : "text-[#23262F] hover:text-[#141718]"
              )}
            >
              <ArrowLeft size={18} strokeWidth={2.5} />
            </button>

            {/* Middle Vertical Separator Divider */}
            <div className="w-[1.5px] h-5 bg-[#E6E8EC]" />

            {/* Next / Proceed Button */}
            <button
              onClick={() => slide < OB_SLIDES.length - 1 ? setSlide(slide + 1) : onFinish()}
              className="p-1 hover:scale-105 active:scale-95 transition-all cursor-pointer text-[#23262F] hover:text-[#141718] flex items-center justify-center"
            >
