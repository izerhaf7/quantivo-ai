...
- `ml/llm.py` — `LLMClient` Protocol with `FireworksLLMClient` (production, targets AMD MI300X/MI350 per the hackathon's AMD-hardware requirement — $50 Fireworks credit comes from the AMD AI Developer Program, see `spesifikasi-teknis-boa-saas.md` §3) and `MockLLMClient` (deterministic keyword-based, offline, no API key). Per spec, use a smaller/faster model for `SentimentAgentImpl` (called once per chunk — volume matters) and a larger/stronger model for `SwotAgentImpl`/`SummaryAgentImpl` (called once per report — quality matters); see `ml/llm_demo.py`. The spec's original suggestion (Llama 3.x 8B/70B) is no longer on Fireworks' serverless catalog — `GET /inference/v1/models` against the live account currently returns 7 chat models, none an 8B-class model, so `llm_demo.py` uses `gpt-oss-120b` (small role) and `deepseek-v4-pro` (large role) instead. Re-check `/models` before hardcoding a slug anywhere else — Fireworks' catalog moves.
